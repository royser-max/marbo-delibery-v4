...
# Pedidos del cliente (vista vacía, debes adaptar la consulta a tu modelo)
@app.get("/mis_pedidos", response_class=HTMLResponse)
def mis_pedidos(request: Request):
    with engine.connect() as conn:
        pedidos = []  # Aquí debes armar la consulta real
...
def edit_food(food_id: int, name: str = Form(...), price: float = Form(...)):
    with engine.connect() as conn:
        conn.execute(text("UPDATE food SET name=:name, price=:price WHERE id=:id"), {"name": name, "price": price, "id": food_id})
    return RedirectResponse(url="/owner/menu", status_code=303)
...
def owner_dashboard(request: Request):
    # Consulta pedidos y ventas del día
    with engine.connect() as conn:
    ...
    # Adaptar a formato esperado por Jinja2
...
def reset_day():
    ...
    with engine.connect() as conn:
        ...
        if total > 0:
            ...
            if existing:
                conn.execute(text("UPDATE daily_earnings SET amount=:total WHERE date=:today"), {"total": total, "today": today})
            else:
                conn.execute(text("INSERT INTO daily_earnings (date, amount) VALUES (:today, :total)"), {"today": today, "total": total})
